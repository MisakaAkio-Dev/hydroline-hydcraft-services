...
const mergedId = computed(() => route.params.stationId as string)
...
async function fetchDetail() {
  loading.value = true
  try {
    detail.value = await railwayStore.fetchLocalMergedStationDetail(
      mergedId.value,
      true,
    )
  } catch (error) {
  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between gap-3">
      ...
      <UButton
        color="neutral"
        variant="soft"
        @click="router.push({ name: 'transportation.railway.stations' })"
      >
        返回列表
      </UButton>
    </div>
