...
const props = defineProps<{
  ...
  loading: boolean
  error: string | null
  result: RailwayMergedRouteGeometryRegenerateSummary | null
  routeTitle: string
}>()
...
type TabItem = {
  key: string
  label: string
  result: RailwayRouteGeometryRegenerateResult
}

const resolveTabLabel = (name: string | null | undefined, fallback: string) => {
  const raw = name ?? ''
  const segment = raw.split('||')[1] ?? raw
  const trimmed = segment.trim()
  const normalized = trimmed.split('|').join(' ').trim()
  return normalized || fallback
}

const tabItems = computed<TabItem[]>(() => {
  const list = props.result?.details ?? []
  return list
    .filter((item): item is RailwayRouteGeometryRegenerateResult =>
      Boolean(item),
    )
    .map((item) => {
      const rawName =
        item.routeDiagnostics?.name ?? (item as any).routeName ?? null
      const label = resolveTabLabel(rawName, item.routeId)
      return {
        key: item.routeId,
        label,
        result: item,
...
watch(
  ...
  (items) => {
    ...
    const stillExists = items.some((entry) => entry.key === activeTabKey.value)
    if (!stillExists) {
      activeTabKey.value = items[0].key
    }
  },
...
const activeResult = computed<RailwayRouteGeometryRegenerateResult | null>(
  () => {
    const items = tabItems.value
    if (!items.length) return null
    return (
      items.find((entry) => entry.key === activeTabKey.value)?.result ??
      items[0].result
    )
  },
)
...
const curveDiagnostics = computed(
  () => activeResult.value?.curveDiagnostics ?? null,
)

const missingCurveSegments = computed(
  () => activeResult.value?.missingCurveSegments ?? [],
)

const routeDiagnostics = computed(
  () => activeResult.value?.routeDiagnostics ?? null,
)

const platformDiagnostics = computed(
  () => activeResult.value?.platformDiagnostics ?? [],
)

const routeIds = computed(() => activeResult.value?.routeIds ?? [])
const platformIds = computed(() => activeResult.value?.platformIds ?? [])
const railIds = computed(() => activeResult.value?.railIds ?? [])
const fallbackDiagnostics = computed(
  () => activeResult.value?.fallbackDiagnostics ?? null,
)

const railJobId = computed(() => activeResult.value?.jobId ?? null)
const railRouteId = computed(() => activeResult.value?.routeId ?? null)
const railRouteType = computed(() => activeResult.value?.railwayType ?? null)
  ...
  >
    <template #content>
      <div class="space-y-4 p-6">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">
              {{ routeTitle }} · {{ activeResult?.routeId ?? '—' }}
            </p>
          </div>
        ...
        <div v-if="tabItems.length > 1" class="flex flex-wrap gap-2">
          <button
          ...
          >
            ...
            >
              ...
              >
                ...
                <div>
                  有竖曲线: {{ curveDiagnostics.segmentsWithVerticalCurve }}
                </div>
              </div>
