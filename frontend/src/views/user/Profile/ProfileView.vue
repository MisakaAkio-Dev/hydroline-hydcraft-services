  ...
  <section class="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 pb-16 pt-8">
    ...
    <div class="grid gap-6 md:grid-cols-[1.2fr_1fr]">
      <UCard class="bg-white/80 backdrop-blur-sm dark:bg-slate-900/60">
        <template #header>
          <div class="flex items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-900 dark:text-white">基础信息</h2>
        ...
        <dl class="grid grid-cols-1 gap-4 text-sm md:grid-cols-2">
          <div>
            <dt class="text-xs uppercase tracking-wide text-slate-500">显示名称</dt>
          ...
          <div>
            ...
            <dd class="mt-1 text-slate-900 dark:text-slate-100">{{ auth.user?.email }}</dd>
          </div>
          <div>
            <dt class="text-xs uppercase tracking-wide text-slate-500">PIIC</dt>
            <dd class="mt-1 text-slate-900 dark:text-slate-100">{{ profile.piic ?? '-' }}</dd>
          </div>
          <div>
            <dt class="text-xs uppercase tracking-wide text-slate-500">注册时间</dt>
            <dd class="mt-1 text-slate-900 dark:text-slate-100">
      ...
      <UCard class="bg-white/80 backdrop-blur-sm dark:bg-slate-900/60">
        <template #header>
          <div class="flex items-center justify-between">
            <h2 class="text-lg font-semibold text-slate-900 dark:text-white">联系方式</h2>
        ...
        <div v-if="contacts.length" class="space-y-3">
          ...
          >
            ...
            <p class="text-slate-600 dark:text-slate-300">{{ contact.value }}</p>
          </div>
        </div>
        <div v-else class="flex h-32 items-center justify-center text-sm text-slate-500 dark:text-slate-400">
          暂无联系方式记录。
        </div>
      </UCard>
    </div>
