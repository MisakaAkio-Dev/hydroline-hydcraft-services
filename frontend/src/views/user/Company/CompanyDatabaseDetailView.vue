...
const ownerUser = computed(
  () => company.value?.legalRepresentative || null,
)
...
function getShareholderIdNumber(sh: {
  ...
  if (sh.kind === 'COMPANY') {
    return (
      sh.holderUnifiedSocialCreditCode ||
      sh.holderRegistrationNumber ||
      '—'
    )
  }
  return '—'
      ...
      <div v-else class="space-y-6">
        ...
        <UCard>
          ...
          <dl class="grid gap-x-8 gap-y-4 md:grid-cols-2">
              ...
              <div class="grid grid-cols-[120px_minmax(0,1fr)] gap-3 md:col-span-2">
        ...
        <UCard>
          ...
          <div v-if="(llc?.shareholders?.length ?? 0) === 0" class="text-sm text-slate-500">
            暂无股东及出资信息。
          </div>
          <div v-else class="overflow-x-auto">
            <table class="min-w-[900px] w-full text-sm">
                <thead>
                  <tr class="border-b border-slate-200/70 text-left text-slate-500">
                ...
                <tbody class="divide-y divide-slate-200/60">
                  ...
                  >
                    ...
                    <td class="py-3 pr-3">{{ shareholderIdTypeLabel(sh.kind) }}</td>
                    <td class="py-3 pr-3">
        ...
        <UCard>
          ...
          <div v-if="(llc?.officers?.length ?? 0) === 0" class="text-sm text-slate-500">
            暂无主要人员信息。
          </div>
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                  <div class="truncate text-sm font-medium text-slate-900 dark:text-white">
                    {{ of.user?.displayName || of.user?.name || of.user?.email || '—' }}
                  </div>
