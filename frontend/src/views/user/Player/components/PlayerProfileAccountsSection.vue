...
function getLastLoginLabel(lastLogin: string) {
  const days = dayjs().diff(dayjs(lastLogin), 'day')
  return days <= 0 ? '今天登录过' : `${days} 天前登录过`
}
...
<template>
  <div class="grid grid-cols-1">
    <div class="flex flex-nowrap gap-2 overflow-x-auto p-1 pb-3">
      <template v-if="props.summary">
        <div
          v-for="binding in normalizedBindings"
          :key="binding.id"
          class="w-42 shrink-0 rounded-xl p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 relative hover:shadow-inner hover:outline-primary-400 outline-2 outline-transparent transition duration-200 cursor-pointer"
          role="button"
              ...
              <template #default>
                <div>
                  ...
                  >
                    ...
                    />
                    <span>{{ getLastLoginLabel(binding.lastlogin) }}</span>
                  </div>
                </div>
              </template>
            </UPopover>
