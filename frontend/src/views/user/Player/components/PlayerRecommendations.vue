...
const props = withDefaults(
  ...
  }>(),
  {
    ...
    gridClass: 'grid-cols-2 sm:grid-cols-3 md:grid-cols-10',
    gapClass: 'gap-3',
    itemGapClass: 'gap-2',
    showTypeLabel: true,
  },
...
<template>
  <div class="grid mt-4" :class="[props.gridClass, props.gapClass]">
    <Motion
    ...
    >
      <UTooltip v-if="!props.showTypeLabel" :text="recommendationTooltip(item)">
        <RouterLink
          :to="
            item.type === 'authme'
      ...
      >
        ...
        <p
          class="text-center text-xs font-semibold text-slate-700 dark:text-slate-200 line-clamp-1 truncate break-all w-full"
        >
          {{ item.displayName }}
        </p>
